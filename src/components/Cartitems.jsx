...
const Cartitems = () => {
  ...
  return (
    <div className='border border-white font-roboto'>
      {/* Content */}
      <div className='w-full max-w-screen-lg m-auto py-10 px-4 lg:px-0'>
        ...
        {/* Aqui que os dados do carrinho aparecem */}
        <section className='flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-10 lg:space-y-0 lg:space-x-10 mt-10'>
          ...
          <div className='w-full lg:w-[60%] space-y-3'>
            <div className="overflow-x-auto">
              <table className='w-full min-w-[600px]'>
                ...
                {/* Consumir os dados do context */}
                <tbody>
                  {/* Linha da tabela */}
                  {cart.length === 0 ? (
                    <tr>
                      <td colSpan="4" className='w-full flex justify-center items-center p-4'>
                        <p className='text-lg sm:text-2xl text-[#616161] text-center'>Seu carrinho está vazio.</p>
                      </td>
                    </tr>
                  ) : (
                    cart.map((item, index) => (
                      <tr key={index} className='border-dashed border-b'>
                        <td className='w-[60%] py-5'>
                          {/* Produto */}
                          <div className='flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 space-x-0 sm:space-x-3 py-2'>
                            <img src={item.img} alt="Produto" className='w-24 h-24 sm:w-[100px] sm:h-[100px] bg-white rounded object-cover' />
                            <div>
                        ...
                        <td className='text-center'>
                          <button onClick={() => removeFromCart(item.title)} className='w-full flex items-center justify-center'>
          ...
          {/* Content do lado direito */}
          <div className='w-full lg:w-[40%] h-fit border rounded p-5 space-y-5'>
